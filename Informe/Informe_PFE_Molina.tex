...
% Manejo de enlaces y corte de URLs sin desbordar margen
...
% Paquetes para evitar desbordes de margen (Overfull \hbox)
...
El presente Proyecto Final de Estudios aborda la evolución, reingeniería e integración funcional de una celda de manufactura robotizada a escala para operaciones de clasificación tipo \textit{pick-and-place} dinámico sobre piezas en movimiento. Tomando como plataforma de partida un manipulador serial de 3 GDL con mecanismo de paralelogramo articulado de cuatro barras desarrollado en un proyecto precedente, se diseñó e incorporó una cinta transportadora motorizada independiente y se transformó la operación estática original en un lazo continuo gobernado por visión artificial y control embebido determinista.

Para superar las restricciones de la plataforma heredada, se llevó a cabo una intervención integral a nivel mecánico, electrónico y computacional. En el aspecto físico, se implementaron acoples con unión por chaveta mecanizada en los ejes motores y un esquema de par trenzado diferencial para restaurar la integridad del bus I2C de los encoders magnéticos AS5600. En el plano cinemático, se descartó el modelo previo basado en articulaciones virtuales de Denavit-Hartenberg, formulando una cinemática inversa geométrica directa libre de singularidades espaciales. Asimismo, se reestructuró la arquitectura de control migrando el cálculo analítico de trayectorias quínticas ($C^2$) y el lazo de velocidad \textit{feedforward} + P a 100~Hz directamente a la FPU del microcontrolador STM32F446RE bajo FreeRTOS, comunicándose mediante micro-ROS con el host de supervisión y una nueva interfaz gráfica interactiva en PyQt5.

La percepción cenital en OpenCV (30~FPS, espacio HSV) introduce un esquema de compuertas espaciales consecutivas sobre la cinta que estima la velocidad lineal de avance en tiempo real ($v_{\text{obj}}$) sin depender de la carga gráfica del supervisor, complementado con un algoritmo de discriminación geométrica invariante a la orientación (Cubo vs. Cono) mediante la relación de envolventes (\textit{Rect Ratio}). Un filtro analítico de factibilidad cinemática evalúa en tiempo de ejecución si el tiempo disponible de arribo satisface los límites dinámicos de aceleración del robot ($T_{\text{MIN\_ROBOT}}$), sincronizando la intercepción mediante una rutina de despegue vertical puro ($Z_{\text{lift}}$) que suprime esfuerzos de corte.

La validación experimental sobre la celda demostró una tasa de éxito del 100\% en modo estático ($N=23$), alcanzando un 77.1\% de efectividad en la captura dinámica y un 74.3\% de ciclo completo clasificado en régimen continuo ($N=35$, $\bar{v} = 75.9\text{ mm/s}$). El sistema demostró capacidad de intercepción exitosa hasta los $108.9\text{ mm/s}$, a partir de la cual el orquestador aborta preventivamente las trayectorias inviables preservando el sincronismo de los actuadores y garantizando la seguridad global de la celda.

\vfill
\noindent \textbf{Palabras clave:} Robótica de Manipulación, Pick-and-Place Dinámico, ROS 2, micro-ROS, FreeRTOS, Visión Artificial, Cinemática Inversa.
...
\begin{tikzpicture}[
    ...
    auto,
    >=Stealth,
    block/.style={rectangle, draw=black!80, fill=white, thick, text width=4.8cm, align=center, rounded corners=3pt, minimum height=1.2cm},
    hwblock/.style={rectangle, draw=teal!80!black, fill=teal!8, thick, text width=4.3cm, align=center, rounded corners=3pt, minimum height=1.2cm},
    rtosblock/.style={rectangle, draw=orange!80!black, fill=orange!10, thick, text width=4.8cm, align=center, rounded corners=3pt, minimum height=1.2cm},
    mathblock/.style={rectangle, draw=violet!80!black, fill=violet!10, thick, text width=4.3cm, align=center, rounded corners=3pt, minimum height=1.2cm},
    ...
    \node [rtosblock, below=1.3cm of ros] (defaulttask) {
        \textbf{\texttt{tasks.c} (\texttt{defaultTask})}\\
        \scriptsize micro-ROS Executor + UART DMA
    };
    
    \node [rtosblock, below=1.4cm of defaulttask] (controltask) {
    ...
    % Columna izquierda (ampliado a 2.2cm para dar margen limpio a las líneas y textos)
    \node [hwblock, left=2.2cm of motors] (sensors) {
        \textbf{\texttt{as5600\_tca9548.c}}\\
        \scriptsize Interrupción TIM7 + MUX I2C\\
        \scriptsize 3x Encoders AS5600 (400 kHz)
    };

    % 1. Conexiones verticales del tronco central
    \draw [line] (ros.south) -- node[lbl] {Trama UART DMA} (defaulttask.north);
    \draw [line] (defaulttask.south) -- node[lbl] {Cola \texttt{mid\_PositionQueue}} (controltask.north);
    \draw [line] (controltask.south) -- node[lbl] {$q_{ref}, \dot{q}_{ref}$} (motors.north);

    % 2. Conexiones horizontales con planificador y cinemática
 ...
 % 3. Telemetría AS5600: Entrada local a defaultTask (desde sensors)
    \draw [dashedline, rounded corners=6pt] (sensors.north) 
        -- ++(0, 1.8cm)
        -- node[rotate=90, above=6pt, midway, font=\scriptsize, text=black!85] {Telemetría 40 Hz} ++(0, 2.2cm) 
        |- ([yshift=-3mm]defaulttask.west);

    % 4. Flujo de traj_status local: mControlTask -> defaultTask
    \draw [dashedline, rounded corners=6pt] (controltask.west) 
        -- ++(-1.0cm, 0) 
        -- node[rotate=90, above=5pt, midway, font=\scriptsize, text=black!85] {\texttt{traj\_status}} ++(0, 2.2cm)
        |- ([yshift=3mm]defaulttask.west);

    % 5. ÚNICA línea de publicación desde defaultTask hacia ROS 2 con los dos tópicos
    \draw [dashedline, rounded corners=6pt] (defaulttask.west) 
        -- ++(-2.0cm, 0) 
        -- node[rotate=90, above=6pt, midway, font=\scriptsize, text=black!85, align=center] {
            Publica \texttt{/microROS/angles}\\
            y \texttt{/planner/traj\_status}
        } ++(0, 2.7cm)
        |- (ros.west);
\end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Callback central alineado exactamente con el ejecutor
        \node[box, text width=3.0cm, below=1.3cm of executor] (cbP2p) {\texttt{p2p\_cmd\_callback}\\Joints Directos};
        
        % Callbacks laterales simétricos
        \node[box, text width=3.0cm, left=0.4cm of cbP2p] (cbCmd) {\texttt{cmd\_callback}\\Trama Cartesiana};
        \node[box, text width=3.0cm, right=0.4cm of cbP2p] (cbEstop) {\texttt{estop\_callback}\\Parada Emergencia};

        \node[queue, below=0.7cm of cbCmd] (queue) {\texttt{mid\_PositionQueue}};

        % Conexiones limpias y simétricas desde executor
        \draw[->, rounded corners=6pt] (executor.south) -- ++(0,-0.4) -| (cbCmd.north);
        \draw[->] (executor.south) -- (cbP2p.north);
        \draw[->, rounded corners=6pt] (executor.south) -- ++(0,-0.4) -| (cbEstop.north);
...
\centering
\small
\begin{tabularx}{\linewidth}{|p{3.0cm}|p{2.1cm}|p{2.5cm}|X|}
\hline
...
\begin{itemize}
    \item \textbf{\texttt{kinematics.c}:} Geometría de cuatro barras: $L_1 = 77.0\text{ mm}$, $L_2 = 135.0\text{ mm}$, $L_3 = 147.0\text{ mm}$, $O_r = 25.49\text{ mm}$, $O_z = 40.87\text{ mm}$. Límites articulares de seguridad unificados estrictamente con el hardware: $Q_1 \in [-90^\circ, 90^\circ]$, $Q_2 \in [20^\circ, 157^\circ]$ (cota inferior requerida para alcanzar la cota lejana en $Y = -140.0\text{ mm}$), $Q_3 \in [-75^\circ, 25^\circ]$.
...
\begin{itemize}
    ...
    \end{equation}
    donde $a = 2.78487 \times 10^{-5}$, $b = 0.599751$ y $c = -193.5176$. El eje transversal de la cinta se encuentra posicionado en $X_{\text{FIXED}} = 215.0\text{ mm}$ y la altura de contacto en $Z_{\text{FIXED}} = 76.0\text{ mm}$.
    
    \item \textbf{Estimación Lineal de Velocidad por Compuertas Fijas:}
    Configura dos barreras espaciales transversales a la cinta: $Y_{G1} = -140.0\text{ mm}$ y $Y_{G2} = -70.0\text{ mm}$ ($\Delta Y = 70.0\text{ mm}$). Al cruzar la pieza $Y_{G1}$, registra la marca de tiempo de alta resolución $t_1$ (\texttt{time.perf\_counter()}) junto a la coordenada observada $y_1$. Al cruzar $Y_{G2}$, calcula la velocidad escalar determinista a partir del desplazamiento real observado:
    \begin{equation}
        v_{cinta} = \frac{y_2 - y_1}{t_2 - t_1} \quad [\text{mm/s}]
    \end{equation}
...
\begin{itemize}
    ...
    \end{equation}
    Para el trayecto completo nominal desde la guardia ($X=215, Y=-75, Z=100\text{ mm}$) hasta la intercepción ($X=215, Y=120, Z=76\text{ mm}$), este cálculo arroja $T_{\text{MIN\_ROBOT}} \approx 0.85\text{ s}$. Si la velocidad observada impone un tiempo disponible menor ($t_{disp} < T_{\text{MIN\_ROBOT}}$), la máquina de estados aborta preventivamente la intercepción. Esto evita que el manipulador inicie trayectorias inviables donde los motores perderían sincronismo al intentar acelerar por encima de $850\text{ Hz}$.
...
\paragraph{\texttt{ros\_thread.py} y \texttt{robot\_ui\_node.py} --- Desacoplamiento Gráfico}
...
\centering
\small
\begin{tabularx}{\linewidth}{|p{3.1cm}|p{2.2cm}|p{2.5cm}|X|}
\hline
...
\subsection{Mapeo Métrico por Ajuste Polinómico (Píxel a Milímetro)} 
...
\begin{itemize}
    \item \textbf{Compuerta de Entrada ($Y\_1 = -140.0\text{ mm}$):} Registra la marca temporal de cruce $t_1$ mediante \texttt{time.perf\_counter()} junto a la coordenada observada $y_1$.
    \item \textbf{Compuerta de Medición y Disparo ($Y\_2 = -70.0\text{ mm}$):} Registra la marca temporal $t_2$ y la posición $y_2$ al completar el recorrido nominal fijo $\Delta Y = 70.0\text{ mm}$.
\end{itemize}

La velocidad lineal estimada del objeto se obtiene directamente a partir del diferencial observado en los fotogramas de cruce:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Filtro Analítico en ROS 2 ($T\_{\text{MIN\_ROBOT}}$):} 
    ...
    \item \textbf{Cota Inferior del Generador Quíntico en Firmware ($t\_{\text{min\_fw}} = 0.60\text{ s}$):} 
...
\begin{itemize}
    \item \textbf{Elevación Vertical Pura ($Z\_{\text{lift}} = 125.0\text{ mm}$):} El efector asciende verticalmente manteniendo fijas las coordenadas horizontales ($X=215.0\text{ mm}, Y=120.0\text{ mm}$) durante $0.6\text{ s}$, despegando la pieza de la cinta antes de iniciar el giro.
...
5. Validación Experimental Cuantitativa & Cumplido & Tasa de éxito en agarre del 77.1\% (27/35) y efectividad global de ciclo Pick+Place clasificado del 74.3\% (26/35) en intercepción dinámica continua, con 100\% en modo estático ($N=23$). \\ \hline
...
Se dispuso el objeto en coordenadas cartesianas conocidas dentro del espacio de trabajo sobre la cinta detenida ($X = 215.0\text{ mm}$, $Z = 76.0\text{ mm}$, $Y \in [+10, +140]\text{ mm}$). Se realizaron 23 repeticiones evaluando el agarre magnético y la correcta clasificación hacia el depósito.
...
\hline
\textbf{N$^\circ$} & \textbf{$Y\_{\text{pick}}$ [mm]} & \textbf{Pick} & \textbf{Place} & \textbf{Clasificación} \\ \hline
...
\centering
\scriptsize
\begin{tabular}{|c|c|c|c|l|p{5.0cm}|}
\hline
...
\begin{itemize}
    \item Sobre los \textbf{35 ciclos operativos válidos}, el robot logró completar la maniobra de (\textit{Pick} exitoso) en \textbf{27 ocasiones}, representando una \textbf{tasa de éxito del 77.14\%}.
    \item De los 27 casos donde se ejecutó un \textit{Pick} exitoso, en 1 ocasión se registró un fallo de clasificación (\textit{Place} $\times$), lo que representa un \textbf{3.7\%} de fallos en la clasificación morfológica.
    \item En los 8 casos donde falló el (\textit{Pick} $\times$), representando una \textbf{tasa de error del 22.9\%}, se observó que 4 de ellos fueron consecuencia de una captura errónea de la velocidad, lo que representa un \textbf{11.4\%} de los casos, mientras que los 4 restantes se debieron a errores de agarre por fricción insuficiente, también un \textbf{11.4\%} de los casos.
    \item El rango de velocidad lineal registrado exitosamente osciló entre $65.6\text{ mm/s}$ y $88.9\text{ mm/s}$, con un promedio de $\bar{v} = 75.9\text{ mm/s}$.
\end{itemize}
