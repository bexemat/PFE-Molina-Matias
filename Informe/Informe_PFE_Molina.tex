...
\begin{itemize}
    ...
    \item \textbf{Detalle 2 (Figura~\ref{fig:diagrama_supervisor_detalle2}):} Detalla las transiciones del ciclo continuo: validación de posición de guardia ($\pm 2.0\text{ mm}$), activación del electroimán, cálculo de intercepción MRU hacia $Y = +120.0\text{ mm}$, elevación vertical pura ($Z = 125.0\text{ mm}$ para evitar corte por fricción) y verificación óptica continua de agarre con retorno evasivo rápido ante pérdidas de pieza.
...
\hline
Meta Intercepción & $Y = +120.0$ mm & \texttt{sm\_v2.py} & Posición predecible en espacio de trabajo. \\
\hline
...
\subsection{Modelo de Sincronización Temporal e Intercepción en $Y = +120\text{ mm}$}
Con el objeto cruzando la cota de disparo ($y_{\text{disparo}} = Y_2$), la distancia restante hasta la meta fija de intercepción ($Y_{\text{pick}} = +120.0\text{ mm}$) es conocida analíticamente:
\begin{equation}
...
\begin{enumerate}
    \item Si $t_{\text{arribo}} < T_{\text{min}}$, el tiempo disponible es insuficiente para que el manipulador alcance la coordenada $Y = +120.0\text{ mm}$ de forma segura.
...
3. Procesamiento Digital de Imágenes & Cumplido & Detección cenital a 30 FPS en espacio HSV, clasificación invariante Cubo/Cono por Rect Ratio y estimación de velocidad por compuertas espaciales. \\ \hline
...
5. Validación Experimental Cuantitativa & Cumplido & Tasa de éxito global del 86.7\% en modo dinámico y 100\% en estático, con análisis de repetibilidad y límites de velocidad. \\ \hline
...
Para evaluar el desempeño global de la celda de manufactura, se definieron cuatro casos de uso representativos de la operación industrial. Los ensayos se registraron audiovisualmente y se procesaron estadísticamente a partir de series de repeticiones continuas.

\section{Caso de Uso 1: Pick-and-Place Estático con Retroalimentación Visual}
\subsection{Descripción del Escenario}
Se dispuso el objeto en 10 coordenadas cartesianas discretas conocidas dentro del espacio de trabajo sobre la cinta detenida ($X = 220\text{ mm}$, $Z = 50\text{ mm}$, $Y \in [-80, +80]\text{ mm}$). Se realizaron 30 repeticiones evaluando el error posicional euclídeo de agarre y el tiempo total de ciclo.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Parámetro Evaluado} & \textbf{Mínimo} & \textbf{Promedio} & \textbf{Máximo} & \textbf{Desv. Est. ($\sigma$)} \\
\midrule
Error posicional $X$ [mm] & 0.4 & 1.1 & 1.8 & 0.35 \\
Error posicional $Y$ [mm] & 0.2 & 0.8 & 1.4 & 0.28 \\
Tiempo de ciclo total [s] & 3.2 & 3.5 & 3.9 & 0.18 \\
Tasa de agarre exitoso & \multicolumn{4}{c}{\textbf{100\%} (30/30 ensayos)} \\
\bottomrule
\end{tabular}
\caption{Métricas obtenidas en el Caso de Uso 1: Pick-and-Place Estático.}
\label{tab:resultados_estatico}
\end{table}

\section{Caso de Uso 2: Clasificación Geométrica Invariante a la Orientación}
\subsection{Descripción del Escenario}
Se hicieron circular 40 muestras (20 cubos y 20 conos truncados) introducidas de manera aleatoria con distintos ángulos de desalineación respecto al eje longitudinal de la cinta transportadora ($0^\circ$ a $90^\circ$).
...
\begin{tabular}{lccc}
\toprule
\textbf{Geometría} & \textbf{Muestras} & \textbf{Clasificación Correcta} & \textbf{Efectividad (\%)} \\
\midrule
Cubo (Alineado $0^\circ \pm 15^\circ$) & 10 & 10 & 100\% \\
Cubo (Rotado en diagonal $45^\circ \pm 15^\circ$) & 10 & 10 & 100\% \\
Cono Truncado (Circular) & 20 & 19 & 95\% \\
\midrule
\textbf{Total Global} & \textbf{40} & \textbf{39} & \textbf{97.5\%} \\
\bottomrule
\end{tabular}
\caption{Matriz de confusión de clasificación morfológica con lazo combinado Extent / Rect Ratio.}
\label{tab:resultados_clasificacion}
\end{table}

\section{Caso de Uso 3: Intercepción Dinámica Síncrona a Velocidad Nominal}
\subsection{Descripción del Escenario}
Se reguló la cinta a su velocidad nominal de operación ($v_{\text{cinta}} \approx 85\text{ mm/s}$). El manipulador ejecutó el ciclo continuo en 1 paso: estimación por compuertas ($Y_1 \rightarrow Y_2$), planificación quíntica en FPU, intercepción en $Y = +120\text{ mm}$, elevación vertical ($Z_{\text{lift}}$) y depósito clasificado.
...
\midrule
Captura, elevación y clasificación exitosa & 26 & 86.7\% \\
Fallo por desprendimiento durante despegue ($Z$) & 2 & 6.7\% \\
Fallo por error de sincronismo temporal ($\Delta t > 80\text{ ms}$) & 2 & 6.7\% \\
\midrule
\textbf{Tasa de Éxito Global (Success Rate)} & \multicolumn{2}{c}{\textbf{86.7\%}} \\
\bottomrule
\end{tabular}
\caption{Rendimiento cuantitativo en intercepción dinámica continua ($N = 30$).}
\label{tab:resultados_dinamico}
\end{table}

\section{Caso de Uso 4: Límites Operativos de Velocidad y Rechazo Cinemático}
\subsection{Descripción del Escenario}
Se sometió el sistema a un barrido progresivo de velocidades en la cinta transportadora desde $40\text{ mm/s}$ hasta $220\text{ mm/s}$ en incrementos de $20\text{ mm/s}$ para verificar el filtro de factibilidad cinemática ($T_{\text{min}} = 0.85\text{ s}$) y el retorno evasivo.

\subsection{Resultados Cuantitativos}
Se comprobó que el rango de operación óptimo se sitúa entre $50\text{ mm/s}$ y $110\text{ mm/s}$. Para velocidades de cinta superiores a $140\text{ mm/s}$, el algoritmo de descarte cinemático abortó correctamente el 100\% de las operaciones riesgosas, evitando colisiones mecánicas y pérdidas de paso en los motores.

\section{Registro Audiovisual y Demostraciones}
Para facilitar la evaluación externa y la verificación de los resultados obtenidos, los registros audiovisuales de los ensayos se encuentran estructurados según los casos de uso:
\begin{itemize}
    \item \textbf{Video Anexo 1 (Pick-and-Place Estático):} Validación del lazo cerrado de visión y repetibilidad posicional del efector final.
    \item \textbf{Video Anexo 2 (Intercepción Dinámica Continua V2):} Secuencia completa de detección en compuertas, intercepción síncrona en movimiento, despegue vertical y clasificación en bandejas.
    \item \textbf{Video Anexo 3 (Failsafe y Retorno Rápido):} Demostración del comportamiento de la máquina de estados ante desprendimiento de pieza y rechazo automático por velocidad excesiva.
\end{itemize}
...
\begin{itemize}
    ...
    \item Integrar un encoder incremental sobre el eje motriz de la cinta para medir fluctuaciones de velocidad en tiempo real causadas por rozamiento o variaciones de carga.
    \item Implementar redes neuronales embebidas (TinyML) para la clasificación de objetos mediante microcontroladores especializados con aceleración de hardware.
\end{itemize}
